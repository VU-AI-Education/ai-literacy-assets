class QuizApp {
    ...
    loadQuestion() {
        ...
        this.questionCounter.textContent = `Vraag ${this.currentQuestion + 1} van ${quizData.length}`;
    ...
    showFeedback() {
        ...
        if (this.isCorrect) {
            ...
            this.feedbackMessage.textContent = 'Goed gedaan! Je hebt het juiste antwoord.';
            ...
            this.feedbackMessage.textContent = 'Dat is niet helemaal juist. Laten we meer leren over dit concept.';
    ...
    showResults() {
        ...
        if (percentage >= 90) {
            performanceMessage = 'Uitstekend! Je hebt een sterk begrip van generatieve AI-concepten.';
        } else if (percentage >= 70) {
            performanceMessage = 'Goed gedaan! Je hebt een solide begrip van de meeste concepten.';
        } else if (percentage >= 50) {
            performanceMessage = 'Niet slecht! Er is ruimte voor verbetering op sommige gebieden.';
        } else {
            performanceMessage = 'Overweeg om het materiaal nog eens door te nemen en het opnieuw te proberen.';
        }
        
        this.scoreBreakdown.innerHTML = `
            <p>${performanceMessage}</p>
            <p style="margin-top: 12px; font-size: 1rem;">
                Juiste antwoorden: ${this.score}<br>
                Onjuiste antwoorden: ${quizData.length - this.score}
            </p>
        `;
    }
